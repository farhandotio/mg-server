...
const sendOrderEmails = async (order, userEmail, userName) => {
  try {
    await sendEmail({
      ...
      message: `Hi ${userName}, Thank you for your order! Your order for BDT ${order.pricing.totalPrice} has been placed successfully.`,
      buttonText: 'View Order Status',
      buttonUrl: `${process.env.FRONTEND_URL}/account/orders/${order._id}`,
    });
    ...
    await sendEmail({
      ...
      buttonUrl: `${process.env.FRONTEND_URL}/admin/orders/${order._id}`,
    });
  } catch (error) {
...
export const createOrder = async (req, res) => {
  try {
    ...
    const order = await orderModel.create({
      ...
      pricing: { itemsPrice, shippingPrice, totalPrice },
      orderStatus: payment.method === 'COD' ? 'CONFIRMED' : 'PENDING',
    });
    ...
    sendOrderEmails(order, userEmail, userName);
